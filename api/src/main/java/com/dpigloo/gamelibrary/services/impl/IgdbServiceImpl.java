...
public class IgdbServiceImpl implements IgdbService {
  ...
  public GameDto getGameById(long id) throws RequestException {
    APICalypse apiCalypse = new APICalypse().fields("name, id, cover.image_id, involved_companies.company.name, involved_companies.developer, genres.name").where("id = " + id);
    logger.info(apiCalypse.buildQuery());
    ...
    List<String> developers = result.getInvolvedCompaniesList().stream().filter(InvolvedCompany::getDeveloper).map(InvolvedCompany::getCompany).map(Company::getName).toList();
    List<String> genres = result.getGenresList().stream().map(Genre::getName).toList();
    ...
    gameDto.setDeveloper(developers);
